  ...
  implements Item
{
    ...
    throws ParsingException
  {
    ...
    if (paramFetchResponse.peekByte() == 40)
    {
      ...
      do
      {
        localVector.addElement(new BODYSTRUCTURE(paramFetchResponse));
        paramFetchResponse.skipSpaces();
      }
      while (paramFetchResponse.peekByte() == 40);
      this.bodies = new BODYSTRUCTURE[localVector.size()];
      localVector.copyInto(this.bodies);
        ...
        do
        {
          while (true)
          {
            ...
            else
            {
              ...
              throw new ParsingException("BODYSTRUCTURE parse error: " + this.type + "/" + this.subtype + ": " + "bad multipart disposition, b " + j);
              if (k != 32)
              ...
              while (paramFetchResponse.readByte() == 32)
              {
                ...
                continue;
                String str2 = paramFetchResponse.readString();
                if (str2 != null)
                {
                  this.language = new String[] { str2 };
                  if (parseDebug)
                    System.out.println("DEBUG IMAP: language " + str2);
                }
              }
            }
          }
          ...
          else if ((this.type.equalsIgnoreCase("message")) && (this.subtype.equalsIgnoreCase("rfc822")))
          {
            ...
            BODYSTRUCTURE[] arrayOfBODYSTRUCTURE = new BODYSTRUCTURE[1];
            arrayOfBODYSTRUCTURE[0] = new BODYSTRUCTURE(paramFetchResponse);
    ...
    label1525: if (paramFetchResponse.peekByte() == 40)
    {
      this.language = paramFetchResponse.readStringList();
      if (parseDebug)
        System.out.println("DEBUG IMAP: language len " + this.language.length);
    ...
    while (true)
    {
      if (paramFetchResponse.readByte() != 32)
      {
        ...
        return;
        String str1 = paramFetchResponse.readString();
        if (str1 == null)
          continue;
        this.language = new String[] { str1 };
        if (!parseDebug)
          continue;
        System.out.println("DEBUG IMAP: language " + str1);
        continue;
      }
      parseBodyExtension(paramFetchResponse);
    }
