...
cc.Class({
    ...
    _fetchRoomConfigs: function() {
        ...
        try {
            ...
            // 请求 API
            console.log("📡 [房间配置] 请求API: " + apiUrl + '/api/v1/room/config/list');
            HttpAPI.get(
                apiUrl + '/api/v1/room/config/list',
                cryptoKey,
                function(err, result) {
                    if (err) {
                        console.warn("❌ [房间配置] API请求失败:", err);
                        self._initRoomButtons(self._getDefaultRoomConfigs());
                        return;
                    }
                    
                    console.log("✅ [房间配置] API响应:", JSON.stringify(result).substring(0, 500));
                    ...
                    if (configs && configs.length > 0) {
                        // 调试：输出每个竞技场配置的开赛时间和每场时长
                        for (var i = 0; i < configs.length; i++) {
                            var c = configs[i];
                            if (c.room_category === 2 || c.roomCategory === 2) {
                                console.log("🏟️ [竞技场配置] ID=" + c.id + ", Name=" + (c.room_name || c.roomName) + 
                                    ", matchTimeRanges=" + (c.match_time_ranges || c.matchTimeRanges) + 
                                    ", matchDuration=" + (c.match_duration || c.matchDuration || c.match_round_duration || c.matchRoundDuration));
                            }
                        }
                        self.roomConfigs = configs;
                        self._initRoomButtons(configs);
                    } else {
                        console.warn("⚠️ [房间配置] 未获取到配置数据");
                        self._initRoomButtons(self._getDefaultRoomConfigs());
                    }
                }
            );
    ...
    _renderRoomLayout: function(allRooms) {
        ...
        // ============================================================
        // 参数设置 - 每个竞技场对应一个文字框，宽度与卡片一致
        // ============================================================
        var cardWidth = 180;       // 卡片宽度（默认值）
        var cardHeight = 120;      // 卡片高度（默认值）
        var gapX = 30;             // 卡片水平间距
        ...
        var actualCardWidths = [];
        
        for (var i = 0; i < allRooms.length; i++) {
            var room = allRooms[i];
            var actualWidth = room.node.width || cardWidth;
            actualCardWidths.push(actualWidth);
            totalCardsWidth += actualWidth;
        }
        totalCardsWidth += (allRooms.length - 1) * gapX;
        
        // 使用最大的卡片高度
        var maxCardHeight = cardHeight;
        for (var i = 0; i < allRooms.length; i++) {
            var h = allRooms[i].node.height || cardHeight;
            if (h > maxCardHeight) maxCardHeight = h;
        }
        
        var panelWidth = Math.max(totalCardsWidth + 40, screenWidth - 100);
        var panelHeight = maxCardHeight + 40;
        ...
        // 放置所有卡片 - 水平排列
        var currentX = -totalCardsWidth / 2;
        for (var i = 0; i < allRooms.length; i++) {
            var room = allRooms[i];
            var actualWidth = actualCardWidths[i];
            ...
            // 卡片水平位置：从左到右排列，使用实际卡片宽度
            room.node.x = currentX + actualWidth / 2;
            currentX += actualWidth + gapX;
            // 卡片垂直位置：居中
            room.node.y = 0;
    ...
    _addArenaSignupButtons: function() {
        ...
        for (var i = 0; i < this._arenaRooms.length; i++) {
            ...
            // 背景尺寸 - 每个竞技场对应一个文字框，宽度与卡片一致
            var bgWidth = btnNode.width;         // 宽度: 与竞技场卡片宽度一致
            var bgHeight = 64;                   // 高度: 64
            var bgRadius = 16;                   // 圆角: 16
            var bgColor = cc.color(201, 109, 46, 255);  // 颜色: #C96D2E
            ...
            // 位置：在卡片内部顶部显示
            roomStatusItem.x = btnNode.x;
            roomStatusItem.y = btnNode.y + btnNode.height / 2 - bgHeight / 2;
            ...
            // ========== 3. 第二行文字：未到开赛时间（DescLabel）==========
            var descLabel = new cc.Node("DescLabel");
            var descLabelComp = descLabel.addComponent(cc.Label);
            descLabelComp.string = "未到开赛时间";
            descLabelComp.fontSize = 12;
            descLabelComp.lineHeight = 16;
            descLabelComp.horizontalAlign = cc.Label.HorizontalAlign.CENTER;
            descLabelComp.verticalAlign = cc.Label.VerticalAlign.CENTER;
            descLabel.color = cc.color(255, 244, 208);  // #FFF4D0
            descLabel.anchorX = 0.5;
            descLabel.anchorY = 0.5;
            descLabel.y = -10;  // y: -10
            descLabel.parent = roomStatusItem;
            ...
            // 设置按钮大小 - 根据图片比例(300x200)缩放，宽度与卡片匹配
            var cardWidth = btnNode.width || 180;
            var btnWidth = Math.min(cardWidth * 0.70, 120);  // 按钮宽度为卡片宽度的70%，最大120
            var btnHeight = btnWidth * (140 / 300);  // 按钮高度降低，更矮一点
            signupBtn.setContentSize(cc.size(btnWidth, btnHeight));
            signupBtn.anchorX = 0.5;
            signupBtn.anchorY = 0.5;  // 锚点设为中心
            
            // 位置：按钮紧贴卡片底部，保持约5px间距
            signupBtn.x = btnNode.x;
            signupBtn.y = btnNode.y - btnNode.height / 2 + btnHeight / 2 - 5;
    ...
    _canSignupArena: function(config) {
        ...
        // 情况1：没有开赛时间，没有每场时长 → 禁用
        if (!matchTimeRanges && !matchDuration) {
            return false;
        }
        
        // 情况2：没有开赛时间，有每场时长 → 允许（24小时开放）
        if (!matchTimeRanges && matchDuration) {
            return true;
        }
        ...
        // 情况4：有开赛时间，有每场时长 → 检查是否在开赛时间段内
        return this._isInMatchTime(config);
    },
    ...
    _updateCountdownDisplay: function() {
        ...
        for (var i = 0; i < this._arenaRooms.length; i++) {
            ...
            var descLabel = roomStatusItem.getChildByName("DescLabel");
            ...
            // 判断是否可以报名
            var canSignup = this._canSignupArena(config);
            
            // 获取最近的时段信息
            var timeRangeInfo = this._getNearestMatchTimeRange(config);
            var countdownInfo = this._getNextMatchCountdown(config);
            
            // 更新开赛时间段显示（第一行）
            if (titleLabel) {
                var titleLabelComp = titleLabel.getComponent(cc.Label);
                if (matchTimeRanges && timeRangeInfo) {
                    if (timeRangeInfo.inRange) {
                        // 当前在开赛时间段内
                        titleLabelComp.string = "开赛中 " + this._formatMatchTimeRange(timeRangeInfo.range);
                    } else {
                        // 不在开赛时间段，显示最近的时段
                        titleLabelComp.string = "下场 " + this._formatMatchTimeRange(timeRangeInfo.range);
                    }
                } else if (!matchTimeRanges && matchDuration) {
                    // 24小时开放
                    titleLabelComp.string = "24小时开放";
                } else {
                    // 配置不完整
                    titleLabelComp.string = "暂未开放";
                }
            ...
            // 更新倒计时显示（第二行）
            if (descLabel) {
                var descLabelComp = descLabel.getComponent(cc.Label);
                
                if (!canSignup) {
                    // 不能报名的情况
                    if (!matchTimeRanges && !matchDuration) {
                        descLabelComp.string = "未配置开赛时间";
                    } else if (matchTimeRanges && !matchDuration) {
                        descLabelComp.string = "未配置每场时长";
                    } else if (matchTimeRanges && !countdownInfo.inMatchTime) {
                        descLabelComp.string = "不在开赛时间";
                    } else {
                        descLabelComp.string = "暂不可报名";
                    }
                } else if (countdownInfo.inMatchTime) {
                    // 可以报名且在开赛时间，显示倒计时
                    descLabelComp.string = "下场 " + this._formatCountdown(countdownInfo.seconds);
                } else {
                    descLabelComp.string = "等待开赛";
                }
                // 使用淡金色
                descLabel.color = cc.color(255, 244, 208);
            }
