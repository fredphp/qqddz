...
func TestStartGame_DealCards(t *testing.T) {
        ...
        // Verify state
        assert.Equal(t, GameStateCallLandlord, gs.state)
...
func TestStartGame_BidderSelected(t *testing.T) {
        ...
        // Verify a caller was selected (0, 1, or 2)
        assert.GreaterOrEqual(t, gs.callIndex, 0)
        assert.Less(t, gs.callIndex, 3)
}
...
func TestNewGameSession_Initialization(t *testing.T) {
        ...
        assert.Equal(t, -1, gs.firstCallerIdx) // 初始状态没有抢地主的人
...
func TestGameSession_PlayerOfflineHandling(t *testing.T) {
        ...
        // Mark player as offline (using State field)
        gs.mu.Lock()
        gs.players[0].State = PlayerStateOffline
        gs.mu.Unlock()

        // Verify offline status
        gs.mu.RLock()
        assert.Equal(t, PlayerStateOffline, gs.players[0].State)
        gs.mu.RUnlock()
}
