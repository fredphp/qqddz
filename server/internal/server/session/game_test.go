...
func TestHandleBid_Success(t *testing.T) {
        ...
        // Get current caller
        currentCaller := gs.players[gs.callIndex]

        // Bid successfully (抢地主)
        err := gs.HandleCallLandlord(currentCaller.ID, "call")
        require.NoError(t, err)

        // Verify landlord is set
        assert.Equal(t, GameStatePlaying, gs.state)
        assert.True(t, currentCaller.IsLandlord)
}
...
func TestHandleBid_NotYourTurn(t *testing.T) {
        ...
        wrongIdx := (gs.callIndex + 1) % 3
        wrongPlayer := gs.players[wrongIdx]
        err := gs.HandleCallLandlord(wrongPlayer.ID, "call")
...
func TestHandleBid_AllPass_RandomLandlord(t *testing.T) {
        ...
        for range 3 {
                currentCaller := gs.players[gs.callIndex]
                err := gs.HandleCallLandlord(currentCaller.ID, "pass")
                require.NoError(t, err)
        }

        // Verify a landlord was assigned (第一个玩家成为地主)
        assert.Equal(t, GameStatePlaying, gs.state)
        landlordCount := 0
