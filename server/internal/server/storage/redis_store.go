...
const (
        ...
        availableRoomsKey   = "rooms:available" // 可加入房间列表（少于3人且至少有1人）
...
// RoomListItem 房间列表项（用于 Redis 序列化）
...
// AddToAvailableRooms 添加房间到可加入列表
func (rs *RedisStore) AddToAvailableRooms(ctx context.Context, item *RoomListItemData) error {
        jsonData, err := json.Marshal(item)
        if err != nil {
                return fmt.Errorf("序列化房间列表项失败: %w", err)
        }
        return rs.client.RPush(ctx, availableRoomsKey, jsonData).Err()
}

// RemoveFromAvailableRooms 从可加入列表移除房间
func (rs *RedisStore) RemoveFromAvailableRooms(ctx context.Context, roomCode string) error {
        // 获取列表中所有房间
        items, err := rs.client.LRange(ctx, availableRoomsKey, 0, -1).Result()
        if err != nil {
                return err
        }
        ...
        for _, item := range items {
                var data RoomListItemData
                if err := json.Unmarshal([]byte(item), &data); err != nil {
                        continue
                }
                if data.RoomCode == roomCode {
                        return rs.client.LRem(ctx, availableRoomsKey, 1, item).Err()
                }
        }
        return nil
}

// UpdateAvailableRoom 更新可加入列表中的房间信息
func (rs *RedisStore) UpdateAvailableRoom(ctx context.Context, item *RoomListItemData) error {
        // 先移除旧的
        if err := rs.RemoveFromAvailableRooms(ctx, item.RoomCode); err != nil {
                return err
        }
        // 如果房间还有位置，重新添加
        if item.PlayerCount < item.MaxPlayers {
                return rs.AddToAvailableRooms(ctx, item)
        }
        return nil
}

// GetAvailableRooms 获取可加入的房间列表
func (rs *RedisStore) GetAvailableRooms(ctx context.Context) ([]RoomListItemData, error) {
        items, err := rs.client.LRange(ctx, availableRoomsKey, 0, -1).Result()
        if err != nil {
                return nil, err
        }
...
// ClearAvailableRooms 清空可加入房间列表
func (rs *RedisStore) ClearAvailableRooms(ctx context.Context) error {
        return rs.client.Del(ctx, availableRoomsKey).Err()
}
