        ...
        if room.State == RoomStateWaiting && isCreator {
                ...
                for _, player := range room.Players {
                        if player.Client != nil {
                                player.Client.SendMessage(codec.NewErrorMessageWithText(protocol.ErrCodeUnknown, "房主已离开，房间已解散"))
                                player.Client.SetRoom("")
                        }
                }

                // 更新数据库房间状态为已关闭
                if err := database.UpdateRoomStatus(roomCode, database.RoomStatusClosed); err != nil {
