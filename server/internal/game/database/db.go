...
func UpdatePlayerGold(playerID uint64, goldChange int64) error {
        return UpdatePlayerGoldWithTx(nil, playerID, goldChange)
}

// UpdatePlayerGoldWithTx 在事务中更新玩家金币
// 🔧【修复】支持事务传递，避免锁竞争
func UpdatePlayerGoldWithTx(tx *gorm.DB, playerID uint64, goldChange int64) error {
        if playerID == 0 {
                log.Printf("⚠️ [UpdatePlayerGold] playerID 为 0，跳过更新")
                return nil
        }
        ...
        if result.Error != nil {
                log.Printf("❌ [UpdatePlayerGold] 更新玩家 %d 金币失败: %v", playerID, result.Error)
                return result.Error
        }
        
        if result.RowsAffected == 0 {
                log.Printf("⚠️ [UpdatePlayerGold] 未找到玩家 %d，没有行被更新", playerID)
        } else {
                log.Printf("✅ [UpdatePlayerGold] 玩家 %d 金币更新成功，变化: %d，影响行数: %d", playerID, goldChange, result.RowsAffected)
        }

        return nil
}

// UpdatePlayerArenaCoin 更新玩家竞技币
func UpdatePlayerArenaCoin(playerID uint64, coinChange int64) error {
        if playerID == 0 {
                log.Printf("⚠️ [UpdatePlayerArenaCoin] playerID 为 0，跳过更新")
                return nil
        }

        result := DB().Model(&Player{}).
                Where("id = ?", playerID).
                Update("arena_coin", gorm.Expr("arena_coin + ?", coinChange))

        if result.Error != nil {
                log.Printf("❌ [UpdatePlayerArenaCoin] 更新玩家 %d 竞技币失败: %v", playerID, result.Error)
                return result.Error
        }

        if result.RowsAffected == 0 {
                log.Printf("⚠️ [UpdatePlayerArenaCoin] 未找到玩家 %d，没有行被更新", playerID)
        } else {
                log.Printf("✅ [UpdatePlayerArenaCoin] 玩家 %d 竞技币更新成功，变化: %d", playerID, coinChange)
        }

        return nil
}

// 🔧【新增】UpdatePlayerArenaCoinWithLog 更新玩家竞技币并记录流水
// changeType: 变化类型，参考 ArenaCoinChange* 常量
// relatedID: 关联ID（如期号、游戏ID等）
// remark: 备注说明
func UpdatePlayerArenaCoinWithLog(playerID uint64, coinChange int64, changeType uint8, relatedID, remark string) error {
        if playerID == 0 {
                log.Printf("⚠️ [UpdatePlayerArenaCoinWithLog] playerID 为 0，跳过更新")
                return nil
        }
        ...
        return DB().Transaction(func(tx *gorm.DB) error {
                ...
                // 2. 获取更新后的余额
                var player Player
                if err := tx.Select("arena_coin").Where("id = ?", playerID).First(&player).Error; err != nil {
                        return err
                }

                // 3. 创建流水记录
                coinLog := &ArenaCoinLog{
                        PlayerID:     playerID,
                        ChangeAmount: coinChange,
                ...
                if err := tx.Create(coinLog).Error; err != nil {
                        log.Printf("❌ [UpdatePlayerArenaCoinWithLog] 创建竞技币流水失败: %v", err)
                        return err
                }

                log.Printf("✅ [UpdatePlayerArenaCoinWithLog] 玩家 %d 竞技币更新成功，变化: %d，余额: %d，流水ID: %d",
                        playerID, coinChange, player.ArenaCoin, coinLog.ID)
...
// UpdatePlayerStatsWithTx 在事务中更新玩家统计数据
// 🔧【修复】支持事务传递，避免锁竞争
func UpdatePlayerStatsWithTx(tx *gorm.DB, playerID uint64, win bool, isLandlord bool) error {
...
func SaveGameResult(record *GameRecord, dealLogs []DealLog, bidLogs []BidLog, playLogs []PlayLog) error {
        ...
        return DB().Transaction(func(tx *gorm.DB) error {
                ...
                if err := UpdatePlayerGoldWithTx(tx, record.Farmer1ID, record.Farmer1WinGold); err != nil {
                        return err
                }
                ...
                if err := UpdatePlayerStatsWithTx(tx, record.Farmer1ID, !landlordWin, false); err != nil {
                        return err
                }
