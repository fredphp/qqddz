...
type Player struct {
        ...
        PlayerType   uint8          `gorm:"type:tinyint;not null;default:1;index;comment:玩家类型:1-真人,2-机器人" json:"player_type"`
        ...
        // 机器人状态字段
        RobotStatus            uint8      `gorm:"type:tinyint;not null;default:0;comment:机器人状态:0-空闲,1-竞技场中" json:"robot_status"`
        RobotCurrentSessionID  *uint64    `gorm:"type:bigint unsigned;comment:当前竞技场会话ID" json:"robot_current_session_id"`
        RobotLockedAt          *time.Time `gorm:"type:datetime;comment:机器人锁定时间" json:"robot_locked_at"`
...
// PlayerType 玩家类型
const (
        PlayerTypeHuman  uint8 = 1 // 真人玩家
        PlayerTypeRobot  uint8 = 2 // 机器人
)

// RobotStatus 机器人状态
const (
        RobotStatusIdle    uint8 = 0 // 空闲
        RobotStatusInArena uint8 = 1 // 竞技场中
)
...
type GoldLog struct {
        ...
        ChangeAmount int64      `gorm:"type:bigint;not null;comment:变化金额(正数为获得,负数为消耗)" json:"change_amount"`
        BalanceAfter int64      `gorm:"type:bigint;not null;comment:变化后余额" json:"balance_after"`
        ChangeType   uint8      `gorm:"type:tinyint;not null;comment:变化类型:1-游戏结算,2-系统赠送,3-广告奖励,4-其他" json:"change_type"`
        RelatedID    string     `gorm:"type:varchar(64);default:'';comment:关联ID(游戏ID等)" json:"related_id"`
        Remark       string     `gorm:"type:varchar(256);default:'';comment:备注" json:"remark"`
        CreatedAt    time.Time  `gorm:"type:datetime;not null;default:CURRENT_TIMESTAMP;index;comment:创建时间" json:"created_at"`
...
// IsGain 是否获得（正数）
func (l *GoldLog) IsGain() bool {
        return l.ChangeAmount > 0
}

// IsCost 是否消耗（负数）
func (l *GoldLog) IsCost() bool {
        return l.ChangeAmount < 0
}
...
// AdRewardLog 广告奖励日志表模型
type AdRewardLog struct {
        ID           uint64    `gorm:"primaryKey;autoIncrement;comment:日志ID" json:"id"`
        PlayerID     uint64    `gorm:"type:bigint unsigned;not null;index;comment:玩家ID" json:"player_id"`
        AdType       string    `gorm:"type:varchar(32);not null;comment:广告类型:bean/arena_coin" json:"ad_type"`
        RewardAmount int64     `gorm:"type:bigint;not null;comment:奖励数量" json:"reward_amount"`
        CurrencyType string    `gorm:"type:varchar(32);not null;comment:货币类型:gold/arena_coin" json:"currency_type"`
        CreatedAt    time.Time `gorm:"type:datetime;not null;default:CURRENT_TIMESTAMP;index;comment:创建时间" json:"created_at"`
