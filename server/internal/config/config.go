...
type APIConfig struct {
        Enable      bool   `yaml:"enable"`       // 是否启用HTTP API
        Port        int    `yaml:"port"`         // HTTP API端口
        CryptoKey   string `yaml:"crypto_key"`   // 加密密钥（32字节用于AES-256）
        EnableCrypto bool  `yaml:"enable_crypto"` // 是否启用加密
}
...
type CDNConfig struct {
        URL string `yaml:"url"` // CDN域名地址，用于补全图片URL
}
...
func loadFromEnv(cfg *Config) {
        ...
        // API
        getEnvInt("API_PORT", &cfg.API.Port)
        getEnvStr("API_CRYPTO_KEY", &cfg.API.CryptoKey)

        // MySQL
        getEnvStr("MYSQL_HOST", &cfg.MySQL.Host)
        getEnvInt("MYSQL_PORT", &cfg.MySQL.Port)
        getEnvStr("MYSQL_USER", &cfg.MySQL.User)
        getEnvStr("MYSQL_PASSWORD", &cfg.MySQL.Password)
        getEnvStr("MYSQL_DATABASE", &cfg.MySQL.Database)

        // CDN
        getEnvStr("CDN_URL", &cfg.CDN.URL)
}
...
func setDefaults(cfg *Config) {
        ...
        // API
        setDefaultInt(&cfg.API.Port, 1781)

        // MySQL
        setDefaultStr(&cfg.MySQL.Host, "127.0.0.1")
        setDefaultInt(&cfg.MySQL.Port, 3306)
        setDefaultStr(&cfg.MySQL.User, "root")
        setDefaultStr(&cfg.MySQL.Database, "hlddz")

        // CDN - 默认为空，表示不使用CDN
        setDefaultStr(&cfg.CDN.URL, "")
}
