...
func (h *AuthHandler) PhoneLogin(w http.ResponseWriter, r *http.Request) {
        ...
        */
        ...
        // 使用CDN补全头像URL
        avatarUrl := cdnutil.CompleteAvatar(player.Avatar)
        log.Printf("✅ 玩家头像URL: %s (原始: %s)", avatarUrl, player.Avatar)
...
func (h *AuthHandler) mockPhoneLogin(w http.ResponseWriter, phone string) {
        ...
        // 使用CDN补全默认头像URL
        avatarUrl := cdnutil.CompleteAvatar("avatar_1")
...
func (h *AuthHandler) WxLogin(w http.ResponseWriter, r *http.Request) {
        ...
        // 使用CDN补全头像URL
        avatarUrl := cdnutil.CompleteAvatar(req.Avatar)
...
func (h *AuthHandler) WxAppLogin(w http.ResponseWriter, r *http.Request) {
        ...
        // 使用CDN补全头像URL
        avatarUrl := cdnutil.CompleteAvatar(req.AvatarUrl)
...
// GetBalance 获取玩家余额
func (h *AuthHandler) GetBalance(w http.ResponseWriter, r *http.Request) {
        ...
        if token == "" {
                writeJSONError(w, http.StatusBadRequest, "缺少token参数")
                return
        }
        ...
        log.Printf("✅ 获取玩家余额成功 - 玩家ID: %d, 金币: %d, 竞技币: %d", player.ID, player.Gold, player.ArenaCoin)

        writeJSONSuccess(w, map[string]interface{}{
                "gold":       player.Gold,
                "arena_coin": player.ArenaCoin,
        })
}
