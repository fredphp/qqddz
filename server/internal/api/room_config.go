...
type RoomConfigResponse struct {
        ...
        MatchRoundDuration int    `json:"matchDuration"`  // 每场时长(分钟)
...
// RedisClientWrapper Redis客户端包装器（适配go-redis）
type RedisClientWrapper struct {
        client *redis.Client
}

// NewRedisClientWrapper 创建Redis客户端包装器
...
// Set 设置值
func (w *RedisClientWrapper) Set(ctx context.Context, key string, value string, expiration time.Duration) error {
...
// SIsMember 检查是否是集合成员
func (w *RedisClientWrapper) SIsMember(ctx context.Context, key string, member interface{}) (bool, error) {
        return w.client.SIsMember(ctx, key, member).Result()
}

// 确保RedisClientWrapper实现RedisClient接口
...
func (h *RoomConfigHandler) ClearCache() {
        if h.redis != nil {
                ...
                err := h.redis.Del(ctx, RedisCacheKeyRoomConfigList)
                if err != nil {
                        log.Printf("❌ Redis房间配置缓存清除失败: %v", err)
                } else {
                        log.Printf("✅ Redis房间配置缓存已清除, key: %s", RedisCacheKeyRoomConfigList)
                }
        } else {
                log.Println("⚠️ Redis未初始化，无法清除缓存")
        }
}
...
func (h *RoomConfigHandler) GetActiveRoomConfigs(w http.ResponseWriter, r *http.Request) {
                ...
                if err == nil && cached != "" {
                        ...
                        if jsonErr := json.Unmarshal([]byte(cached), &configs); jsonErr == nil {
                                ...
                                for _, c := range configs {
                                        log.Printf("📋 缓存配置: ID=%d, Name=%s, Category=%d, matchTimeRanges='%s', matchDuration=%d",
                                                c.ID, c.RoomName, c.RoomCategory, c.MatchTimeRanges, c.MatchRoundDuration)
                                        // 如果是竞技场房间（Category=2）但没有开赛时间配置，需要刷新缓存
                                        // 修复：只要 matchTimeRanges 为空就应该刷新，不管 matchDuration 的值
                                        if c.RoomCategory == 2 && c.MatchTimeRanges == "" {
                                                log.Printf("⚠️ 竞技场房间 %s 缓存数据缺少开赛时间配置(matchTimeRanges为空)，需要刷新", c.RoomName)
                                                needRefresh = true
                                        }
                                }
        ...
        // 2. Redis缓存未命中，从数据库查询
        log.Println("🔄 Redis缓存未命中或无效，从数据库查询...")
...
func (h *RoomConfigHandler) cacheToRedis(configs []RoomConfigResponse) {
        ...
        if data, err := json.Marshal(configs); err == nil {
                ...
                // 调试日志：输出缓存内容摘要
                for _, c := range configs {
                        log.Printf("📝 已缓存: ID=%d, Name=%s, matchTimeRanges='%s', matchDuration=%d",
                                c.ID, c.RoomName, c.MatchTimeRanges, c.MatchRoundDuration)
                }
...
func (h *RoomConfigHandler) getRoomConfigsFromGORM() ([]RoomConfigResponse, error) {
        ...
        for _, rc := range roomConfigs {
                // 调试日志：输出竞技场配置字段
                log.Printf("📋 房间配置: ID=%d, Name=%s, Category=%d, MatchTimeRanges='%s', MatchRoundDuration=%d",
                        rc.ID, rc.RoomName, rc.RoomCategory, rc.MatchTimeRanges, rc.MatchRoundDuration)
                ...
                configs = append(configs, RoomConfigResponse{
                        ...
                        // 竞技场专属字段
                        MatchTimeRanges:    rc.MatchTimeRanges,
                        MatchRoundDuration: rc.MatchRoundDuration,
                        MatchRoundCount:    rc.MatchRoundCount,
                        MaxPlayers:         rc.MaxPlayers,
                        MinPlayers:         rc.MinPlayers,
                        ChampionRewardID:   rc.ChampionRewardID,
                })
        }
        ...
        // 调试日志：输出最终返回的配置数据
        for _, c := range configs {
                log.Printf("📤 响应配置: ID=%d, Name=%s, Category=%d, matchTimeRanges='%s', matchDuration=%d",
                        c.ID, c.RoomName, c.RoomCategory, c.MatchTimeRanges, c.MatchRoundDuration)
        }
...
func (h *RoomConfigHandler) getRoomConfigsFromSQL() ([]RoomConfigResponse, error) {
        query := `SELECT id, room_name, room_type, room_category, base_score, multiplier, min_gold, max_gold,
                  ...
                  match_time_ranges, match_round_duration, match_round_count, max_players, min_players, champion_reward_id
        ...
        for rows.Next() {
                ...
                var matchTimeRanges sql.NullString
                var championRewardID sql.NullInt64
                err := rows.Scan(
                        &config.ID,
                        &config.RoomName,
                        &config.RoomType,
                ...
                // 竞技场字段
                if matchTimeRanges.Valid {
                        config.MatchTimeRanges = matchTimeRanges.String
                }
                if championRewardID.Valid {
                        config.ChampionRewardID = uint(championRewardID.Int64)
                }
