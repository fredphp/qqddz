...
-- 竞技币流水API (不存在于 hlddz.sql)
INSERT INTO `sys_apis` (`created_at`, `updated_at`, `path`, `description`, `api_group`, `method`)
SELECT NOW(), NOW(), '/ddz/arenaCoinLog/list', '获取竞技币流水列表', '斗地主竞技币流水', 'POST'
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `sys_apis` WHERE `path` = '/ddz/arenaCoinLog/list' AND `method` = 'POST');

-- 金币流水API (不存在于 hlddz.sql)
INSERT INTO `sys_apis` (`created_at`, `updated_at`, `path`, `description`, `api_group`, `method`)
SELECT NOW(), NOW(), '/ddz/goldLog/list', '获取金币流水列表', '斗地主金币流水', 'POST'
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `sys_apis` WHERE `path` = '/ddz/goldLog/list' AND `method` = 'POST');
