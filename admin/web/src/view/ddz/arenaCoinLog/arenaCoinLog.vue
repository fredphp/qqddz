  ...
  <div class="arena-coin-log-page">
    ...
    <div class="table-section">
      ...
      >
        ...
        <el-table-column align="center" label="变化前余额" min-width="130">
          <template #default="scope">
            <span class="coin-amount">{{ formatNumber(scope.row.beforeBalance) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" label="变化后余额" min-width="130">
          <template #default="scope">
            <span class="coin-amount">{{ formatNumber(scope.row.balanceAfter) }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="变化类型" min-width="110">
          <template #default="scope">
        ...
        <el-table-column align="center" label="备注" min-width="180" show-overflow-tooltip>
          <template #default="scope">
...
import { Search, Refresh, User, Trophy, Top, Bottom, Document, Clock } from '@element-plus/icons-vue'
...
.stat-card--total::before { background: linear-gradient(180deg, #f093fb 0%, #f5576c 100%); }
.stat-card--income::before { background: linear-gradient(180deg, #11998e 0%, #38ef7d 100%); }
.stat-card--expense::before { background: linear-gradient(180deg, #667eea 0%, #764ba2 100%); }
.stat-card--count::before { background: linear-gradient(180deg, #4facfe 0%, #00f2fe 100%); }
...
.stat-card--total .stat-card__icon { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); }
.stat-card--income .stat-card__icon { background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); }
.stat-card--expense .stat-card__icon { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); }
.stat-card--count .stat-card__icon { background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); }
