...
// 🔧【统一修复】以下所有房间配置方法统一使用 ddz_room_config 表
// 注意：DDZRoomConfig 模型对应 ddz_room_config 表（与游戏服务端一致）
// DDZRoomConfigs 模型对应 ddz_room_configs 表（已废弃，不再使用）

// GetRoomConfigList 获取房间配置列表（ddz_room_config 表）
// 🔧【修复】统一使用 DDZRoomConfig 模型（ddz_room_config 表）
...
// CreateRoomConfig 创建房间配置（ddz_room_config 表）
// 🔧【修复】统一使用 DDZRoomConfig 模型（ddz_room_config 表）
func (s *DDZConfigService) CreateRoomConfig(req ddzReq.DDZRoomConfigCreate) error {
        ...
        config := ddz.DDZRoomConfig{
                ...
                RoomCategory: req.RoomLevel, // RoomLevel 映射到 RoomCategory
                BaseScore:    req.BaseScore,
                MinGold:      req.MinCoins,
                MaxGold:      req.MaxCoins,
...
// UpdateRoomConfig 更新房间配置（ddz_room_config 表）
// 🔧【修复】统一使用 DDZRoomConfig 模型（ddz_room_config 表）
func (s *DDZConfigService) UpdateRoomConfig(req ddzReq.DDZRoomConfigUpdate) error {
        ...
        if req.RoomLevel > 0 {
                updates["room_category"] = req.RoomLevel
        }
        if req.BaseScore > 0 {
                updates["base_score"] = req.BaseScore
        }
        updates["min_gold"] = req.MinCoins
        updates["max_gold"] = req.MaxCoins
...
// DeleteRoomConfig 删除房间配置（ddz_room_config 表）
// 🔧【修复】统一使用 DDZRoomConfig 模型（ddz_room_config 表）
func (s *DDZConfigService) DeleteRoomConfig(id uint) error {
        db := GetDDZDB()
        err := db.Delete(&ddz.DDZRoomConfig{}, id).Error
        if err != nil {
                return err
        }
...
// 🔧【修复】GORM 会自动过滤软删除记录（因为 DDZRoomConfig 包含 gorm.DeletedAt 字段）
// 不需要手动添加 deleted_at IS NULL 条件，只过滤 status = 1
func (s *DDZConfigService) RefreshRoomConfigCache() error {
        ...
        db := GetDDZDB()
        var configs []ddz.DDZRoomConfig
        // 🔧【修复】GORM 会自动添加 deleted_at IS NULL 条件
        if err := db.Where("status = 1").Order("sort_order ASC").Find(&configs).Error; err != nil {
                global.GVA_LOG.Error("查询房间配置失败", zap.Error(err))
                return err
        }

        // 添加调试日志
        global.GVA_LOG.Info("🔄 [RefreshRoomConfigCache] 从数据库查询到房间配置",
                zap.Int("count", len(configs)))
        for _, c := range configs {
                global.GVA_LOG.Info("📋 房间配置",
                        zap.Uint("id", c.ID),
                        zap.String("name", c.RoomName),
                        zap.Int("roomType", c.RoomType),
                        zap.Int("roomCategory", c.RoomCategory),
...
// GetRoomConfigListForAPI 获取启用的房间配置列表（供游戏API调用）
// 🔧【修复】统一使用 DDZRoomConfig 模型（ddz_room_config 表）
// GORM 会自动过滤软删除记录
func (s *DDZConfigService) GetRoomConfigListForAPI() ([]ddz.DDZRoomConfig, error) {
        // 尝试从Redis获取缓存
        if global.GVA_REDIS != nil {
                ctx := context.Background()
