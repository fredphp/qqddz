...
// GetRoomConfigList 获取房间配置列表（ddz_room_configs 表）
...
// CreateRoomConfig 创建房间配置（ddz_room_configs 表）
...
// UpdateRoomConfig 更新房间配置（ddz_room_configs 表）
...
// DeleteRoomConfig 删除房间配置（ddz_room_configs 表）
func (s *DDZConfigService) DeleteRoomConfig(id uint) error {
        db := GetDDZDB()
        return db.Delete(&ddz.DDZRoomConfigs{}, id).Error
}
