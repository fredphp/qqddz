...
func (casbinService *CasbinService) UpdateCasbin(adminAuthorityID, AuthorityID uint, casbinInfos []request.CasbinInfo) error {
        ...
        if global.GVA_CONFIG.System.UseStrictAuth {
                ...
                for i := range casbinInfos {
                        ...
                        if !hasApi {
                                return errors.New("存在api不在权限列表中: " + casbinInfos[i].Path + " " + casbinInfos[i].Method)
                        }
                }
        }
        ...
        // 使用数据库直接删除，确保数据库中的记录被清理干净
        if err := global.GVA_DB.Where("v0 = ?", authorityId).Delete(&gormadapter.CasbinRule{}).Error; err != nil {
                global.GVA_LOG.Error("清除权限失败", zap.Error(err))
                return err
        }
        ...
        if err := global.GVA_DB.Create(&casbinRules).Error; err != nil {
                ...
                return errors.New("添加权限失败: " + err.Error())
        }
