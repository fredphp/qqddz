...
// @Success  200   {object}  response.Response{data=response.PageResult,msg=string}  "获取每日统计"
...
func (api *DDZStatsApi) GetDailyStats(c *gin.Context) {
        ...
        list, total, err := ddzStatsService.GetDailyStatsList(req)
        if err != nil {
        ...
        // 转换为响应格式
        result := make([]ddzRes.DDZDailyStatsResponse, 0, len(list))
        for _, s := range list {
