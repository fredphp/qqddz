...
func (api *DDZGameLogApi) RefreshRoomConfigCache(c *gin.Context) {
        ...
        err := ddzGameLogService.SyncRoomConfigCacheToRedis()
        if err != nil {
                global.GVA_LOG.Error("刷新房间配置缓存失败!", zap.Error(err))
                response.FailWithMessage("缓存刷新失败: "+err.Error(), c)
                return
        }
...
// @Param    month  query     string false "月份(格式: 202401)，默认当月"
...
func (api *DDZGameLogApi) GetRoomDetail(c *gin.Context) {
        ...
        detail, err := ddzGameLogService.GetRoomDetail(roomID, month)
        if err != nil {
...
// GetGameRoomConfigOptions 获取游戏房间配置选项列表
// @Tags     DDZ游戏房间配置
// @Summary  获取游戏房间配置选项列表（用于下拉选择）
// @Security ApiKeyAuth
// @Produce  application/json
// @Success  200  {object}  response.Response{data=[]map[string]interface{}}  "房间选项列表"
// @Router   /ddz/roomConfig/options [get]
func (api *DDZGameLogApi) GetGameRoomConfigOptions(c *gin.Context) {
        options, err := ddzGameLogService.GetGameRoomConfigOptions()
        if err != nil {
                global.GVA_LOG.Error("获取房间选项失败!", zap.Error(err))
                response.FailWithMessage("获取房间选项失败", c)
                return
        }
...
// @Router   /ddz/roomConfig/bg-image-options [get]
func (api *DDZGameLogApi) GetBgImageOptions(c *gin.Context) {
