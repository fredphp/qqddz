...
func (*Local) DeleteFile(key string) error {
        ...
        // 清理 key 中的路径分隔符（只保留文件名）
        key = filepath.Base(key)

        p := filepath.Join(storePath, key)

        global.GVA_LOG.Info("删除文件", zap.String("path", p), zap.String("key", key))

        // 检查文件是否存在
        if _, err := os.Stat(p); os.IsNotExist(err) {
                global.GVA_LOG.Error("文件不存在", zap.String("path", p), zap.Error(err))
        ...
        err := os.Remove(p)
        if err != nil {
                global.GVA_LOG.Error("文件删除失败", zap.String("path", p), zap.Error(err))
                return errors.New("文件删除失败: " + err.Error())
        }

        global.GVA_LOG.Info("文件删除成功", zap.String("path", p))
        return nil
}
