...
func (c *GameServerClient) RefreshRoomConfigCache() error {
        url := fmt.Sprintf("%s/api/internal/cache/refresh/room-config", c.baseURL)
        ...
        global.GVA_LOG.Info("游戏服务器房间配置缓存已刷新", zap.String("url", url))
        return nil
}

// RefreshRoomConfigCache 全局函数，刷新房间配置缓存
func RefreshRoomConfigCache() error {
        client := GetGameServerClient()
        if err := client.RefreshRoomConfigCache(); err != nil {
                global.GVA_LOG.Error("刷新游戏服务器房间配置缓存失败", zap.Error(err))
                return err
        }
        return nil
}
