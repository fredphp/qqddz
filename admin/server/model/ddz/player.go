...
// 玩家状态说明:
// - status: 0-禁用, 1-正常, 2-封禁, 3-冻结
// 玩家类型说明:
// - player_type: 1-平台用户, 2-机器人
type DDZPlayer struct {
        ...
        PlayerType    uint8          `gorm:"type:tinyint;not null;default:1;index;comment:玩家类型:1-平台用户,2-机器人" json:"playerType"`
        ...
        Status        uint8          `gorm:"type:tinyint;not null;default:1;index;comment:状态:0-禁用,1-正常,2-封禁,3-冻结" json:"status"`
        StatusReason  string         `gorm:"type:varchar(500);default:'';comment:状态变更原因" json:"statusReason"`
        StatusExpire  *time.Time     `gorm:"type:datetime;comment:状态过期时间(冻结/封禁到期)" json:"statusExpire"`
