...
type DDZUserAccount struct {
	...
	TokenExpireAt        *string `json:"tokenExpireAt" gorm:"comment:Token过期时间"`
	RefreshToken         string `json:"-" gorm:"comment:刷新Token"`
	RefreshTokenExpireAt *string `json:"refreshTokenExpireAt" gorm:"comment:刷新Token过期时间"`
...
type DDZLoginLog struct {
	global.GVA_MODEL
	PlayerID    uint   `json:"playerId" gorm:"index;comment:玩家ID"`
	AccountID   uint   `json:"accountId" gorm:"index;comment:账户ID"`
	LoginType   int    `json:"loginType" gorm:"comment:登录类型 1手机号 2微信 3游客"`
	LoginResult int    `json:"loginResult" gorm:"comment:登录结果 0失败 1成功"`
	...
	UserAgent   string `json:"userAgent" gorm:"comment:User-Agent"`
